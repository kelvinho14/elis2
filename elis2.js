    ...
    return {
        ...
        rateMouseOutEffect: function(score,isbn){
        	for(var i=1;i<=score;i++){
        		YUI().use('node', 'button', function(Y) {
            		Y.one('#rate_'+isbn+'_'+i).set('src','images/rate_off.png');
        		});
        	}	
        },
        ...
        rateMouseOverEffect: function(score,isbn){
        	for(var i=1;i<=score;i++){
        		YUI().use('node', 'button', function(Y) {
            		Y.one('#rate_'+isbn+'_'+i).set('src','images/rate.png');
        		});
        	}	
        },
        ...
        elis2OpenActDialog: function(ct,isbn,act_name,width,height,visible,header_caption,show_remove,blockid){
        	            ...
        	            success: function (id, result) {
        	            	YUI().use('json-parse', 'json-stringify', function (Y) {
        	            		...
        	            		YUI().use('node', function(Y) {
        	            			...
        	            			block_elis2.initDialog(header_caption,act_name+'_dialog_'+ct,isbn,act_name,width,height,visible,show_remove,blockid);
        ...
        initDialog: function(header_caption,element_id,isbn,header,width,height,visible,show_remove,blockid){
        	...
        	YUI().use('panel','dd-plugin', function (Y) {
        	    ...
        	    if(show_remove==1){
        		    panel.addButton({
        		        ...
        		        action : function (e) {
        		            e.preventDefault();
        		            removeAllItemsConfirm(isbn,blockid);
        		        }
        		    });
        	    }
        	    ...
        	    function elis2RemoveActivity(isbn,blockid){
        	    	var qid = [];
        	    	YUI().use('io','node', 'button', function (Y) {
        	    ...
        	    function removeAllItemsConfirm (isbn,blockid) {
        	        if (nestedPanel) {
        	            ...
        	            buttons: [
        	                {
        	                    ...
        	                    action : function (e) {
        	                        ...
        	                        nestedPanel.hide();
        	                        panel.hide();
        	                        elis2RemoveActivity(isbn,blockid);
        	                        elis2DestoryDialog();
        	                    }
        	                },
        	                {
