...
class elis2_student_form extends moodleform {
    function definition() {
		...
		$PAGE->requires->yui_module('yahoo');
		$PAGE->requires->yui_module('dom');
		$PAGE->requires->yui_module('event');
		$PAGE->requires->yui_module('element');
		$PAGE->requires->yui_module('animation');
		$PAGE->requires->yui_module('dom-event');
		$PAGE->requires->yui_module('ajax');
		...
		if($data->book_search_term==''){
			...
			if($book_obj){
				...
				foreach($book_obj as $book){
					...
					$book_html.='<div>'.($book['thumbnail']==''?elis2_lib::gen_no_cover():'<img src="'.$book['thumbnail'].'">').'</div>';
					$book_html.='<div id="action_div_'.$book['isbn'].'">';
					if($book['isbn']!=''){
						if(in_array($book['isbn'],$my_book_isbn))
							$book_html.='<br/><a href="javascript:block_elis2.removeBookFromList('.$book['isbn'].',\''.$book['id'].'\')"><img src="images/button-cross.png" title="'.get_string('remove_book','block_elis2').'"></a>';
						else {
		...
		$mform->addElement('html',elis2_lib::render_act_summary($data->block_id));
    	...
    	if(sizeof($my_book_obj)>0){
			...
			for($a=0;$a<$my_book_size;$a++){
				$my_book_html .= elis2_lib::gen_mybooklist_record($my_book_obj[$a],$a,$data->block_id);
			}
			$my_book_html .= '</table>';
