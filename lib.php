...
class elis2_lib{
	...
	function get_my_book(){
		global $USER,$DB,$CFG;
		$sql = "SELECT 
				b.gbook_id,b.book_id,b.isbn,b.thumbnail,b.s_thumbnail,b.title,b.authors,b.publisher,
	...
	function get_book_info_with_my_rating($isbn){
		...
		$sql = "SELECT 
					b.book_id,b.isbn,b.thumbnail,b.s_thumbnail,b.title,b.authors,b.publisher,
	...
	function gen_rating_image($small_img=false){
		global $CFG;
		return '<img src="'.$CFG->wwwroot.'/blocks/elis2/images/rate.png" '.($small_img?'width="25px"':'').'>';	
	}
	...
	function gen_rating_off_image($score_value='',$isbn='',$max_rate='',$small_img=false){
		global $CFG;
		$html = '<img src="'.$CFG->wwwroot.'/blocks/elis2/images/rate_off.png" '.($small_img?'width="25px"':'').' ';
		if($max_rate!='')
	...
	function gen_mybooklist_record($obj,$ct='',$block_id){
		global $CFG,$FULLSCRIPT,$data;
		...
		if(isset($obj->is_read) && $obj->is_read==true){
			...
			$my_book_html.= self::gen_activity_icon($obj->isbn,$ct,$block_id);
			$my_book_html.= '</p>';
		}
		$my_book_html.= '</div></td>';
		$my_book_html .= '<td width="10%"><a href="javascript:block_elis2.removeBookFromList('.$obj->isbn.',\''.$obj->gbook_id.'\')"><img src="images/button-cross.png" title="'.get_string('remove_book','block_elis2').'"></a></td>';
		$my_book_html .= '</tr>';
	...
	function gen_activity_icon($isbn='',$ct='',$block_id){
		global $DB,$CFG,$USER;
		$user_yr = $USER->yr==''?USER_YR:$USER->yr;
		...
		for($i=$index;$i<$loop;$i++){
			...
			if(is_file($class_file)){
				...
				if($config->{'elis_'.$act_arr[$i].'_'.$user_yr}){
					...
					$html.= '<a href="javascript:block_elis2.elis2OpenActDialog(\''.$ct.'\',\''.$isbn.'\',\''.$act_arr[$i].'\',800,500,true,\''.${$class_name}->title.'\','.(($submitted_act==$act_arr[$i])?1:0).','.$block_id.');" class="'.$act_arr[$i].'_dialog_btn">'.${$class_name}->render_icon().'</a>';
	...
	function book($title1,$title2,$rec,$css){
        ...
        $row[1]="<td style='border-right:#666666 1px solid;text-align:center;width:190px{$css}' ><b>";
        if($rec!=''){
        ...
        if($rec){
                ...
                $row[2] = "<td style='border-right:#666666 1px solid;text-align:center;width:190px{$css}' ><a href='".$book_link."'>".(isset($rec->s_thumbnail)&&$rec->s_thumbnail!=''?"<img height=150 src='{$rec->s_thumbnail}' />":self::gen_no_cover())."</a></td>";    
            
        }else{
            $row[2] = "<td style='border-right:#666666 1px solid;text-align:center;width:190px{$css}' ><br />No match found<br /></td>";    
        }
        $row[3] = "<td style='border-right:#666666 1px solid;text-align:center;width:190px{$css}' ><i>{$title1}<br />{$title2}</i></td>";
	...
	function render_act_summary($block_id){
		global $DB,$CFG,$USER;
		$user_yr = $USER->yr==''?USER_YR:$USER->yr;
		...
		for($i=0;$i<$act_size;$i++){
			...
			if(is_file($class_file)){
				...
				if($config->{'elis_'.$act_arr[$i].'_'.$user_yr}){
					...
					$count_attempt.= '<td style="font-weight:bold;font-size:22px;color:#000000;text-align:center;padding:0px">'.self::count_submitted_activity(${$class_name}->act_prefix).'</td>'; 
				}
			}
		}
		$icon_html .= '</tr>';
	    
		return '<table>'.$icon_html.$count_attempt.'</table>';	
	}
	...
	function get_reading_report_table($course_id,$from,$to){
		...
		if($reading_size==0){
			...
			foreach($std_arr as $uid=>$std){
				$reading_html.='<tr><td colspan="2">'.$std[0]->name.'</td></tr>';
				...
				for($i=0;$i<$std_read_size;$i++){
					...
					$reading_html.='<tr>
									<td width="30%"><b>'.$std[$i]->title.'</b><br/>'.str_replace('||',', ',$std[$i]->authors).'<br/>'.$std[$i]->borrow_time.'</td>
									<td>'.$act_html.'</td></tr>';
	...
	function get_popular_stats($course_id,$from,$until){
		...
		$participant_obj = elis2_lib::get_course_participants($course_id);
		$sql = "SELECT
					count(b.book_id) as total, b.title
				FROM
					".$CFG->prefix."block_elis2_book AS b
				INNER JOIN
					".$CFG->prefix."block_elis2_booklist AS bl
